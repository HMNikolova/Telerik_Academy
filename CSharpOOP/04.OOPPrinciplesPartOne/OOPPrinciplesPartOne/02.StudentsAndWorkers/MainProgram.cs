...
namespace _02.StudentsAndWorkers
{
    ...
    class MainProgram
    {
        static void Main()
        {
            ...
            {
                new Student("Martin", "Penchev", 11),
                new Student("Victor", "Nikolov", 2),
                new Student("Nikolai", "Nikolov", 12),
                new Student("Hristina", "Marinova", 10),
                new Student("Tanq", "Tincheva", 2),
                new Student("Petko", "Pekov", 7),
                new Student("Dimitar", "Angelov", 4),
                new Student("Dimitar", "Petkov", 5),
                new Student("Vera", "Krasimirova", 9),
                new Student("Katq", "Mircheva", 6)
            };

            var workers = new List<Worker>()
            {
                new Worker("Petar", "Milanov", 40.0m, 8),
                new Worker("Petar", "Martinov", 150.0m, 4),
                new Worker("Meri", "Petrova", 100.0m, 2),
                new Worker("Valq", "Ivanova", 30.0m, 4),
                new Worker("Ivan", "Marinov", 300.0m, 10),
                new Worker("Iulian", "Asenov", 80.0m, 3),
                new Worker("Gosho", "Peshov", 50.0m, 9),
                new Worker("Pesho", "Goshov", 60.0m, 5),
                new Worker("George", "Georgiev", 20.0m, 2),
                new Worker("Maria", "Nedqlkova", 40.0m, 4)
            };

            var sortedGrade = students.OrderBy(x => x.Grade);
            Console.WriteLine("Students ordered by ascending grades");
            foreach (var stud in sortedGrade)
            {
            ...
            var concatGroups = sortedGrade.Concat<Human>(sortBySalary).OrderBy(x => x.FirstName).ThenBy(x => x.LastName);
