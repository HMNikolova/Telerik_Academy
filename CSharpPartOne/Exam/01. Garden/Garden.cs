using System;
    ...
    class Garden
    {
        static void Main()
        {
            ...
            decimal pototoSeed = decimal.Parse(Console.ReadLine());
            decimal pototoArea = decimal.Parse(Console.ReadLine());
            ...
            decimal pototoCost = pototoSeed * 0.25M;
            decimal carrotCost = carrotSeed * 0.6M;
            decimal cabbageCost = cabbageSeed * 0.3M;
            decimal beanCost = beanSeed * 0.4M;

            decimal totalCost = tomatoCost + cucumberCost + pototoCost + carrotCost + cabbageCost + beanCost;

            Console.WriteLine("Total costs: " + totalCost);

            decimal beanArea = 250 - (tomatoArea + cucumberArea + pototoArea + carrotArea + cabbageArea);
            if (beanArea < 0)
            {
